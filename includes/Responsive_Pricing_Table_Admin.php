...
class Responsive_Pricing_Table_Admin
{
    ...
    {
        if ( ! isset( $_POST['pricing_table_box_nonce'] ) ) return;
        if ( ! isset( $_POST['post_type'] ) ) return;
        if ( ! wp_verify_nonce( $_POST['pricing_table_box_nonce'], 'pricing_table_box' ) ) return;
        if ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) return;
        if ( ! current_user_can('edit_pages', $post_id)) return;
        if ('pricing_tables' !== $_POST['post_type']) return;
    ...
    {
        add_meta_box( "pricing-table-manage-plans", __("Manage Packages", "responsive-pricing-table"), array( $this, 'manage_plans' ), "pricing_tables", "normal", "high" );
        add_meta_box( "pricing-table-preview", __("Preview & Usage (Shortcode)", "responsive-pricing-table"), array( $this, 'preview_meta_box' ), "pricing_tables", "normal", "low" );
    }
    ...
    public function preview_meta_box()
    {
        ...
        ?>
        <hr/><p><strong><?php echo __('Copy the following shortcode and paste in post or page where you want to show.'); ?></strong><br>
        <input
            type="text"
